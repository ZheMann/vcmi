...
def balance_patches(unbalanced_dir, balanced_dir):
    ...
    for device in unbalanced_dir.glob('*'):
        ...
        print(f"{device.name} | num patches: {len(patches_dictionary[device.name])}")
...
if __name__ == "__main__":
    unbalanced = Path('/data/s3523799/vbdi/datasets/Exp.III/patches_ds_28D/test/')
    balanced = Path('/data/s3523799/vbdi/datasets/Exp.III/balanced_patches_ds_28D/test/')
